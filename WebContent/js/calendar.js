...
function pickDate(year, month, day)
{
	...
	month	= month < 10 ? '0'+month : month;
        ...
        var ccurDate = new Date();
	var ccurDay = ccurDate.getDate();
	var ccurMonth = ccurDate.getMonth();
	var ccurYear = getRealYear(ccurDate);


        if((day>ccurDay && month>ccurMonth && year>=ccurYear)||(day<ccurDay && month>ccurMonth+1 && year>=ccurYear) )
        { 
         alert('future date cannot beee selected');
         return false;
        }
