                              ...
                              <div class="left-top-corner maxheight">
                                 <div class="right-top-corner maxheight">
                                    <div class="right-bot-corner maxheight">
                                       <div class="left-bot-corner maxheight">
                                         <div class="inner">
                                          <center><h2>Update Product Details</h2></center><hr/>
<span align=right>Fields marked with <span style="color:red;">*</span> are mandatory&nbsp;&nbsp;&nbsp;</span>
<%Product p=new Product();%>
<%p=(Product) request.getSession().getAttribute("products");%>

<%List<BusinessType> bList=new ArrayList<BusinessType>(); %>
<%bList=(List<BusinessType>) request.getSession().getAttribute("busines"); %>

<%List<BusinessType> bLists=new ArrayList<BusinessType>(); %>
<%bLists=(List<BusinessType>) request.getSession().getAttribute("business"); %>

<%List<AdditionalProduct> pList=new ArrayList<AdditionalProduct>(); %>
<%pList=(List<AdditionalProduct>) request.getSession().getAttribute("AddProduct"); %>

<%List<AdditionalProduct> aplist=new ArrayList<AdditionalProduct>(); %>
<%aplist=(List<AdditionalProduct>) request.getSession().getAttribute("AddProducts"); %>

<form id="contacts-form2"  name="Add_Product" action="/NBG/ProductManagementServlet" method="post" onSubmit="return validateformation1();">
<input type="hidden" name="action" value="updatedProductDetails"></input>
<input type="hidden" name="action" value="ViewProduct"></input>
...
<td><input type='text' title="Please Enter Product Name" name="ProductName" maxlength="40" value="<%=p.getProductName() %>" title="Please Enter Product Name" onblur="validateProductName();" id="ProductName" size='30' width='10'></td>
<td><label id="err_message1" style="color: red;"></label></td>
</tr>

<tr align="left" valign="top"> 
<td ><b>Product Description<sup style="color:red;">*</sup> </b></td>
<td><textarea  id="ProductDescription"   name="ProductDescription"  cols='35' rows='4' onblur="validateProductDescription();" title="Please Enter Product Discription" ><%=p.getProductDescription()%></textarea></td>
...
<td><select id="ProductStatus" name="ProductStatus"  title="Please Select Product Status"  onblur="validateProductStatus();" ><option value='select' >---------------Select---------------</option>
...
<td><input type="text" id="ValidFrom" name="ValidFrom" onblur="validateDate();" value="<%=p.getProductValidFrom()%>" title="Please Enter the date" size='30' width='10'  />
...
<%for(BusinessType bt:bLists) {%>
<option style="font-size:13px" value="<%=bt.getBusinessType_name() %>" <% for(BusinessType business:bList){ if(bt.getBusinessType_name().equals(business.getBusinessType_name())){out.println("selected");} }%>><%=bt.getBusinessType_name() %></option>
<%}%>
</select></td>
...
<tr><td></td><td><input type="checkbox" onblur="validationofAdditional();" name="AProduct" <% for(AdditionalProduct aprod:aplist){  if(aproduct.getName().equals(aprod.getName())){out.println("checked");} }%>
...
<tr><td colspan="1"><a href="#" class="button" onClick="return validateformation1();"><em><b>Save</b></em></a></td>
